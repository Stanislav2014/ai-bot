...
6. **Post-merge**
   ...
   - Обновить [context-dump.md](context-dump.md), если задача затронула хотя бы одно из:
     - Flow (новый/изменённый поток, порядок шагов, endpoint, формат логов)
     - Внешнюю зависимость (раздел «Внешние зависимости»: новый сервис, смена протокола/сети, переименование)
     - Карту файлов / модулей (новый модуль, удаление, изменение ответственности)
     - Состав тестов (обновить счётчик `N + M + ... = Total`)
