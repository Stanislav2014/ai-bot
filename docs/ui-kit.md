...
|---------|------------|---------|
| `/start` | Приветствие, показ текущей модели, список команд | [BotHandlers.start](../app/bot/handlers.py) |
| `/help` | Alias на `/start` | [BotHandlers.help_command](../app/bot/handlers.py) |
| `/models` | Inline-клавиатура со всеми установленными моделями | [BotHandlers.models](../app/bot/handlers.py) |
| `/model <name>` | Переключить модель по имени (text-based) | [BotHandlers.set_model](../app/bot/handlers.py) |
| `/reset` | Очистить историю диалога пользователя | [BotHandlers.reset](../app/bot/handlers.py) |

Любой другой текст (не команда) → LLM запрос (с контекстом истории).
